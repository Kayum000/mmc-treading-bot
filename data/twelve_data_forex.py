...
def get_credit_usage() -> dict:
    return {"used": None, "left": None, "limit": None, "provider": "BiQuote", "free": True}


def fetch_api_usage() -> dict:
    req = Request(f"{_BASE_URL}/health", headers={"User-Agent": "mmc-signal-bot/1.0"})
    try:
...
def fetch_forex_candles(symbol: str, interval: str = INTERVAL, outputsize: int = 200) -> pd.DataFrame:
    interval_map = {"1min": "1m", "5min": "5m", "15min": "15m", "30min": "30m", "60min": "1h", "1h": "1h", "4h": "4h", "1d": "1d"}
    ...
    req = Request(f"{_BASE_URL}/api/{safe_symbol}/ohlc?{params}", headers={"User-Agent": "mmc-signal-bot/1.0", "Accept": "application/json"})
    try:
