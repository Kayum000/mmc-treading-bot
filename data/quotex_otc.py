...
def _ssid() -> str:
    value = os.getenv("QUOTEX_SSID") or os.getenv("QUOTEX_SESSION_TOKEN") or ""
    return value.strip()
...
def _origin() -> str:
    return os.getenv("QUOTEX_ORIGIN", "https://qxbroker.com").strip()
...
def _normalise_rows(event_name: str, payload: Any, pending_event: str | None = None):
    ...
    for item in candles:
        try:
            if isinstance(item, dict):
                ...
                continue
            if ts is None or None in (op, hi, lo, cl) or not item_asset:
                continue
...
async def _fetch(asset: str, count: int) -> list[dict[str, Any]]:
    ...
    if not ssid:
        raise RuntimeError(
            "Quotex OTC চালাতে QUOTEX_SSID বা QUOTEX_SESSION_TOKEN Render Secret-এ সেট করতে হবে।"
        )
        ...
        while time.monotonic() < deadline:
            ...
            if isinstance(raw, (bytes, bytearray)):
                if pending_binary_event:
                    try:
                        payload = json.loads(bytes(raw).decode("utf-8", errors="ignore"))
                        rows.extend(_normalise_rows(pending_binary_event, payload, pending_binary_event))
                    except Exception:
                        pass
            ...
            if raw.startswith("0"):
                continue
            if raw == "40":
                await ws.send(_event("authorization", {
                    "session": ssid,
            ...
            if not event_name:
                continue
            if "reject" in str(event_name).lower():
                raise RuntimeError("Quotex SSID rejected or expired; refresh the session token.")
            if event_name in {"s_authorization", "authorization"}:
                await ws.send(_event("instruments/update", {"asset": asset, "period": PERIOD}))
                await ws.send(_event("history/load", {
                    "asset": asset,
                    "index": 0,
                    "period": PERIOD,
                    "time": int(time.time()),
                    "offset": max(3600, count * PERIOD),
                }))
                continue
            if isinstance(payload, dict) and payload.get("_placeholder"):
                pending_binary_event = str(event_name)
                continue
            if event_name in {"candle", "candles", "history/list", "history/load", "candle-generated", "chart_notification/get"}:
                rows.extend(_normalise_rows(event_name, payload))
