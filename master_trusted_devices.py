...
<div id="master-trusted-ui"><button id="mtd-open" type="button" hidden>TRUSTED DEVICES</button><div id="mtd-panel" hidden><button class="mtd-close" id="mtd-close" type="button">×</button><h3>MASTER / TRUSTED DEVICES</h3><div id="mtd-actions"><button class="mtd-action" id="mtd-add" type="button" hidden>ADD TRUSTED</button><button class="mtd-action" id="mtd-self" type="button" hidden>TRUST THIS DEVICE</button><button class="mtd-action" id="mtd-join" type="button" hidden>JOIN TRUSTED</button></div><div id="mtd-list">Checking…</div></div></div>
<script id="master-trusted-ui-script">(function(){
  const host=document.getElementById('master-trusted-ui'),open=document.getElementById('mtd-open'),panel=document.getElementById('mtd-panel'),close=document.getElementById('mtd-close'),add=document.getElementById('mtd-add'),self=document.getElementById('mtd-self'),join=document.getElementById('mtd-join'),list=document.getElementById('mtd-list');
  ...
  open.onclick=()=>{panel.hidden=false;refresh()}; close.onclick=()=>panel.hidden=true;
