...
def generate_signal(frames: dict[str, pd.DataFrame]) -> Signal:
    ...
    if buy_valid and not sell_valid:
        return Signal("BUY", buy, sell, "বুলিশ এমএমসি: ৩০ মিনিট ও ১৫ মিনিটের বাজারের দিক ঊর্ধ্বমুখী এবং একমত। আগের প্রতিরোধের স্তর ভেঙে দাম আবার সেই স্তরে ফিরে এসে সেটিকে সমর্থন হিসেবে ধরে রেখেছে। ৫ মিনিটে কেনার প্রবেশের সংকেতও নিশ্চিত হয়েছে। বহু-সময়সীমার স্কোর এখন সিদ্ধান্তের বাধা নয়; স্কোর শুধু তথ্য হিসেবে দেখানো হচ্ছে।")
    if sell_valid and not buy_valid:
        return Signal("SELL", buy, sell, "বেয়ারিশ এমএমসি: ৩০ মিনিট ও ১৫ মিনিটের বাজারের দিক নিম্নমুখী এবং একমত। আগের সমর্থনের স্তর ভেঙে দাম আবার সেই স্তরে ফিরে এসে সেটিকে প্রতিরোধ হিসেবে ধরে রেখেছে। ৫ মিনিটে বিক্রির প্রবেশের সংকেতও নিশ্চিত হয়েছে। বহু-সময়সীমার স্কোর এখন সিদ্ধান্তের বাধা নয়; স্কোর শুধু তথ্য হিসেবে দেখানো হচ্ছে।")
...
def generate_1m_signal(df: pd.DataFrame) -> Signal:
    ...
    buy_score = int(close > trend) + int(fast > trend) + 2 * int(structure == "bullish_bos") + 2 * int(sweep == "buy_side_rejection") + int(impulse == "bullish") + 2 * int(rejection == "strong_support_rejection")
    sell_score = int(close < trend) + int(fast < trend) + 2 * int(structure == "bearish_bos") + 2 * int(sweep == "sell_side_rejection") + int(impulse == "bearish") + 2 * int(rejection == "strong_resistance_rejection")
    ...
    if buy_valid and not sell_valid:
        return Signal("BUY", buy_score, sell_score, "১ মিনিটের কঠোর এমএমসি: সর্বশেষ বন্ধ হওয়া ক্যান্ডেলে শক্তিশালী support rejection হয়েছে এবং সাম্প্রতিক setup-এ liquidity sweep, displacement ও BOS-এর মধ্যে অন্তত দুটি bullish confirmation পাওয়া গেছে। Trend-ও bullish। তাই পরবর্তী ১ মিনিটের ক্যান্ডেলকে BUY entry হিসেবে ধরা হয়েছে।")
    if sell_valid and not buy_valid:
        return Signal("SELL", buy_score, sell_score, "১ মিনিটের কঠোর এমএমসি: সর্বশেষ বন্ধ হওয়া ক্যান্ডেলে শক্তিশালী resistance rejection হয়েছে এবং সাম্প্রতিক setup-এ liquidity sweep, displacement ও BOS-এর মধ্যে অন্তত দুটি bearish confirmation পাওয়া গেছে। Trend-ও bearish। তাই পরবর্তী ১ মিনিটের ক্যান্ডেলকে SELL entry হিসেবে ধরা হয়েছে।")
    if buy_valid and sell_valid:
        return Signal("NO_TRADE", buy_score, sell_score, "১ মিনিটের শক্তিশালী level ও দুই বা ততোধিক confirmation দুই দিকেই মিলে গেছে; তাই দ্ব্যর্থক অবস্থায় ট্রেড দেওয়া হয়নি।")
    return Signal("NO_TRADE", buy_score, sell_score, "১ মিনিটের কঠোর এমএমসি শর্ত পূরণ হয়নি; পরবর্তী ক্যান্ডেলের জন্য কোনো নিশ্চিত BUY/SELL entry নেই।")
