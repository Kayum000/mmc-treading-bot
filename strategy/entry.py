"""Independent 1m entry engine.

The entry decision is independent from MTF display/bias. MTF is informational;
it must never block an otherwise valid 1m setup.
"""
...
def generate_signal(df):
    """Produce the independent 1m entry signal; MTF is not a gate."""
    if not _valid(df, MIN_HISTORY):
        return Signal("NO_TRADE", 0, 0, "1m entry: insufficient history.")
    ...
    if buy_ok and in_session and not buy_recent:
        return Signal("BUY", 8, 0, f"1m BUY | sweep-low + rejection + confirmation | RR={buy_rr:.2f} | {hour:02d}:xx UTC.")
    if sell_ok and in_session and not sell_recent:
        return Signal("SELL", 0, 8, f"1m SELL | sweep-high + rejection + confirmation | RR={sell_rr:.2f} | {hour:02d}:xx UTC.")
    ...
    return Signal("NO_TRADE", 5 if bias == "BUY" else 0, 5 if bias == "SELL" else 0,
                  f"1m | bias={bias} | waiting for confirmed sweep/displacement | RR(B/S)={buy_rr:.2f}/{sell_rr:.2f} | {session_text} | expansion={expansion:.2f}x.")
