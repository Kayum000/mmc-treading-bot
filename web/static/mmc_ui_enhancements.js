(()=>{
  const KEY='mmc_theme_color', SOUND_KEY='mmc_signal_sound';
  const themes={blue:{a:'#1261c5',b:'#08b887',c:'#58b8f6'},cyan:{a:'#0891b2',b:'#06b6d4',c:'#67e8f9'},purple:{a:'#6d28d9',b:'#8b5cf6',c:'#c4b5fd'},green:{a:'#15803d',b:'#22c55e',c:'#86efac'},orange:{a:'#c2410c',b:'#f97316',c:'#fdba74'}};
  function applyTheme(name){
    const t=themes[name]||themes.blue; let s=document.getElementById('mmc-theme-style');
  ...
  let audio=null;
  function unlockAudio(){try{audio=audio||new(window.AudioContext||window.webkitAudioContext)();if(audio.state==='suspended')audio.resume();window.mmcAudioUnlocked=true}catch(_){}}
  window.enableSignalAudio=unlockAudio;
  ['pointerdown','touchstart','click'].forEach(ev=>document.addEventListener(ev,unlockAudio,{once:true,passive:true}));
  function playSignalSound(){if(!soundEnabled())return;unlockAudio();try{if(!audio)return;const o=audio.createOscillator(),g=audio.createGain(),now=audio.currentTime;o.type='sine';o.frequency.setValueAtTime(880,now);o.frequency.setValueAtTime(1175,now+.09);g.gain.setValueAtTime(.0001,now);g.gain.exponentialRampToValueAtTime(.12,now+.015);g.gain.exponentialRampToValueAtTime(.0001,now+.22);o.connect(g);g.connect(audio.destination);o.start(now);o.stop(now+.23)}catch(_){}}
  function attachSettings(){
    ...
    x.dataset.mmcUiEnhanced='1';const theme=localStorage.getItem(KEY)||'blue',sound=soundEnabled();
    x.insertAdjacentHTML('beforeend',`<label class="setting-row">Theme Color <select id="mmc-theme-color" style="background:#0a2440;color:#eaf4ff;border:1px solid #20486e;border-radius:6px;padding:7px 8px;min-width:120px"><option value="blue">Blue</option><option value="cyan">Cyan</option><option value="purple">Purple</option><option value="green">Green</option><option value="orange">Orange</option></select></label><label class="setting-row">Signal Sound <input type="checkbox" id="mmc-signal-sound" ${sound?'checked':''}></label><button type="button" id="mmc-test-sound" class="mini-btn" style="margin-top:8px;width:100%">Test Signal Sound</button>`);
    const sel=document.getElementById('mmc-theme-color');if(sel){sel.value=theme;sel.addEventListener('change',()=>applyTheme(sel.value))}
