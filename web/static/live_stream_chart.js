(() => {
  ...
  const S={c:null,symbol:'',interval:'1m',bars:[],timer:null,poll:null,clock:null,drawFrame:0,drawQueued:false,lastW:0,lastH:0,scaleMin:null,scaleMax:null,clockOffsetMs:0,marketTs:0};
  const MS=()=>({'1m':60000,'5m':300000,'15m':900000,'30m':1800000,'1h':3600000,'4h':14400000,'1D':86400000}[S.interval]||60000);
  ...
  const ts=t=>{const v=t?.timestamp??t?.Timestamp??t?.lastQuoteAt??t?.LastQuoteAt;if(v==null)return 0;const d=typeof v==='number'?new Date(v<1e12?v*1000:v):new Date(v);return Number.isFinite(d.getTime())?d.getTime():0;};
  const fmtUTC=t=>new Date(t).toISOString().slice(0,19).replace('T',' ')+' UTC';
  const pad=n=>String(Math.max(0,n)).padStart(2,'0');
  const marketNow=()=>S.marketTs>0?S.marketTs+Math.max(0,Date.now()-S.marketTs):Date.now()+S.clockOffsetMs;
  function syncMarketClock(t){const mt=ts(t);if(!mt)return 0;S.marketTs=mt;S.clockOffsetMs=mt-Date.now();return mt;}
  function ensureBar(chart){
    ...
    if(!time){time=document.createElement('span');time.className='chart-candle-time';time.textContent='CANDLE —';bar.appendChild(time);}
  ...
  function updateLiveInfo(){
    ...
    const b=S.bars.at(-1), now=marketNow(), ms=MS(), p=b?.c;
  ...
  function resizeCanvas(c){const r=c.parentElement.getBoundingClientRect(),d=devicePixelRatio||1,w=Math.max(1,r.width),h=Math.max(1,r.height);const pw=Math.floor(w*d),ph=Math.floor(h*d);if(c.width!==pw||c.height!==ph){c.width=pw;c.height=ph;S.lastW=pw;S.lastH=ph;return true;}return false;}
  function drawNow(){
    S.drawQueued=false;
    const c=canvas();if(!c)return;
    resizeCanvas(c);
    const x=c.getContext('2d');x.clearRect(0,0,c.width,c.height);
    const zoom=Math.max(.5,Math.min(4,Number(window.__chartZoom||1))),visible=Math.max(15,Math.min(120,Math.round(90/zoom))),maxOff=Math.max(0,S.bars.length-visible),off=Math.max(0,Math.min(maxOff,Math.round(window.__chartOffset||0))),end=Math.max(15,S.bars.length-off),bs=S.bars.slice(Math.max(0,end-visible),end);
    if(!bs.length){updateLiveInfo();return;}
    const lo=Math.min(...bs.map(b=>b.l)),hi=Math.max(...bs.map(b=>b.h)),sp=Math.max(hi-lo,Math.abs(hi)*1e-6,1e-8),tmin=lo-sp*.1,tmax=hi+sp*.1;
    if(S.scaleMin==null){S.scaleMin=tmin;S.scaleMax=tmax;}else{S.scaleMin+=(tmin-S.scaleMin)*.08;S.scaleMax+=(tmax-S.scaleMax)*.08;}
    const d=devicePixelRatio||1,T=10*d,B=10*d,L=10*d,R=10*d,W=Math.max(1,c.width-L-R),H=Math.max(1,c.height-T-B),y=v=>T+(S.scaleMax-v)/(S.scaleMax-S.scaleMin)*H,gap=W/bs.length,bw=Math.max(2*d,Math.min(14*d,gap*.62));
    bs.forEach((b,i)=>{const xx=L+i*gap+gap/2,up=b.c>=b.o;x.strokeStyle=up?'#22c55e':'#ef4444';x.fillStyle=x.strokeStyle;x.beginPath();x.moveTo(xx,y(b.h));x.lineTo(xx,y(b.l));x.stroke();const top=y(Math.max(b.o,b.c)),bot=y(Math.min(b.o,b.c));x.fillRect(xx-bw/2,top,bw,Math.max(d,bot-top));});
    updateLiveInfo();
  }
  ...
  async function select(v){const s=sym(v);if(!s)return;S.symbol=s;S.bars=[];S.scaleMin=S.scaleMax=null;S.clockOffsetMs=0;S.marketTs=0;window.__chartZoom=1;window.__chartOffset=0;ensureBar(document.getElementById('live-market-chart'));canvas();await history();await connect();}
  function watch(){const pair=document.getElementById('pair');if(!pair){setTimeout(watch,500);return;}const start=()=>{const chart=document.getElementById('live-market-chart');if(!chart){setTimeout(start,500);return;}ensureBar(chart);canvas();pair.onchange=()=>select(pair.value);select(pair.value);const interval=document.getElementById('chart-interval');if(interval){interval.value=S.interval;interval.addEventListener('change',async()=>{S.interval=interval.value||'1m';S.bars=[];S.scaleMin=S.scaleMax=null;await history();await connect();});}window.addEventListener('resize',draw,{passive:true});clearInterval(S.clock);S.clock=setInterval(updateLiveInfo,1000);};start();}
