(() => {
  ...
  function updateLiveInfo(){const b=S.bars.at(-1),now=marketNow(),ms=MS(),p=b?.c;const external=document.getElementById('chart-live');if(external)external.textContent=Number.isFinite(p)?`LIVE ${p.toFixed(5)}`:'LIVE —';const te=document.getElementById('chart-candle-time');if(te)te.textContent=b?.t?`CANDLE ${fmtUTC(b.t)}`:'CANDLE —';const ce=document.getElementById('chart-candle-countdown');const remain=b?.t?Math.max(0,b.t+ms-now):0;if(ce)ce.textContent=b?.t?`CLOSE IN ${pad(Math.floor(remain/60000))}:${pad(Math.floor(remain/1000)%60)}`:'CLOSE IN —';}
  function canvas(){const chart=document.getElementById('live-market-chart'),wrap=chart?.querySelector('.chart-canvas-wrap-inner,.chart-canvas-wrap');if(!wrap)return null;wrap.style.position='absolute';wrap.style.inset='0';let c=wrap.querySelector('canvas#live-stream-canvas');if(!c){c=document.createElement('canvas');c.id='live-stream-canvas';c.style.cssText='position:absolute;inset:0;width:100%;height:100%;z-index:2;cursor:grab;touch-action:none';wrap.appendChild(c);bindDrag(c,wrap);}return c;}
  function resizeCanvas(c){const r=c.parentElement.getBoundingClientRect(),d=devicePixelRatio||1,w=Math.max(1,r.width),h=Math.max(1,r.height),pw=Math.floor(w*d),ph=Math.floor(h*d);if(c.width!==pw||c.height!==ph){c.width=pw;c.height=ph;S.lastW=pw;S.lastH=ph;return true;}return false;}
  function drawNow(){S.drawQueued=false;const c=canvas();if(!c)return;resizeCanvas(c);const x=c.getContext('2d');x.clearRect(0,0,c.width,c.height);const zoom=Math.max(.5,Math.min(4,Number(window.__chartZoom||1))),visible=Math.max(15,Math.min(120,Math.round(90/zoom))),maxOff=Math.max(0,S.bars.length-visible),off=Math.max(0,Math.min(maxOff,Math.round(window.__chartOffset||0))),end=Math.max(15,S.bars.length-off),bs=S.bars.slice(Math.max(0,end-visible),end);if(!bs.length){updateLiveInfo();return;}const lo=Math.min(...bs.map(b=>b.l)),hi=Math.max(...bs.map(b=>b.h)),sp=Math.max(hi-lo,Math.abs(hi)*1e-6,1e-8),tmin=lo-sp*.1,tmax=hi+sp*.1;if(S.scaleMin==null){S.scaleMin=tmin;S.scaleMax=tmax;}else{S.scaleMin+=(tmin-S.scaleMin)*.08;S.scaleMax+=(tmax-S.scaleMax)*.08;}const d=devicePixelRatio||1,T=10*d,B=10*d,L=10*d,R=10*d,W=Math.max(1,c.width-L-R),H=Math.max(1,c.height-T-B),y=v=>T+(S.scaleMax-v)/(S.scaleMax-S.scaleMin)*H,gap=W/bs.length,bw=Math.max(2*d,Math.min(14*d,gap*.62));bs.forEach((b,i)=>{const xx=L+i*gap+gap/2,up=b.c>=b.o;x.strokeStyle=up?'#22c55e':'#ef4444';x.fillStyle=x.strokeStyle;x.beginPath();x.moveTo(xx,y(b.h));x.lineTo(xx,y(b.l));x.stroke();const top=y(Math.max(b.o,b.c)),bot=y(Math.min(b.o,b.c));x.fillRect(xx-bw/2,top,bw,Math.max(d,bot-top));});updateLiveInfo();}
