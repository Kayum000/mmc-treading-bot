...
def _otc_signal(pair: str, automatic: bool) -> dict:
    ...
    # The OTC collector already publishes only fully closed 1-minute candles.
    # Match Real-Market timing: analyze the latest closed candle and make the
    # signal the entry for the current/next 1-minute candle at this boundary.
    # Do not apply a second server-clock filter here; Render clock skew can hide
    # the newest valid closed candle exactly when the minute changes.
