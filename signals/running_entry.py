...
from data.binance_crypto import fetch_crypto_running_candle
...
def get_running_signal(pair: str, market_mode: str = "real") -> dict:
    ...
    if sec < ENTRY_START:
        return {
            "ok": True, "eligible": False, "signal": "WAIT",
            "pair": pair, "market_mode": mode,
            "seconds_until_entry_window": ENTRY_START - sec,
            "seconds_left_in_window": 0,
    ...
    if sec > ENTRY_END:
        return {
            "ok": True, "eligible": False, "signal": "NO_ENTRY",
            "pair": pair, "market_mode": mode,
            "seconds_until_entry_window": 60 - sec + ENTRY_START,
            "seconds_left_in_window": 0,
            "signal_time_utc": now.isoformat(timespec="seconds"),
    ...
    if pending:
        return {
            "ok": True, "eligible": True, "signal": "NO_TRADE",
            "pair": pair, "market_mode": mode,
            "signal_time_utc": now.isoformat(timespec="seconds"),
            "entry_time_utc": now.isoformat(timespec="seconds"),
            "entry_time_bd": _bd(now).strftime("%d %b %Y, %H:%M:%S"),
            "entry_seconds_remaining": 60 - sec,
            "reason": pending,
        }
    ...
    if action in {"BUY", "SELL"}:
        reason = (
            f"{reason} Running 1M candle analysis থেকে {sec}s-এ entry। "
            f"Entry window: {ENTRY_START}-{ENTRY_END}s; 30s-এর পরে নতুন entry বন্ধ।"
        )
    return {
        "ok": True, "eligible": True, "pair": pair, "market_mode": mode,
        "source": "Binance" if mode == "crypto" else "BiQuote",
        "signal": action, "buy_score": result.buy_score, "sell_score": result.sell_score,
        "reason": reason,
        "signal_time_utc": now.isoformat(timespec="seconds"),
        "signal_time_bd": _bd(now).strftime("%d %b %Y, %H:%M:%S"),
        "candle_time": candle_start.isoformat(timespec="seconds"),
        "analysis_candle_time_utc": candle_start.isoformat(timespec="seconds"),
        "entry_price": price, "entry_price_type": "running_1m_price",
        "entry_time_utc": now.isoformat(timespec="seconds"),
        "entry_time_bd": _bd(now).strftime("%d %b %Y, %H:%M:%S"),
        "entry_seconds_remaining": max(0, int((candle_end - now).total_seconds())),
        "expiry_time_utc": candle_end.isoformat(timespec="seconds"),
        "expiry_time_bd": _bd(candle_end).strftime("%d %b %Y, %H:%M:%S"),
        "timeframe": "Running MMC / 1m", "entry_timeframe": "current 1m candle",
        "entry_window": "20-25 seconds", "mmc_level_type": level_info[0] if level_info else None,
