...
from strategy.mmc import Signal, generate_signal, level_for_side, get_mirror_projection, liquidity_sweep, displacement, _dir, _atr
...
def _running_mmc_fallback(frame: pd.DataFrame) -> Signal:
    """Running-candle confirmation when the closed-candle final gate has not fired.

    The fallback never invents a direction: it requires an existing Mirror MMC
    projection, current-candle direction, proximity to that level, and at least
    one live confirmation (sweep, displacement, or structure). This is specific
    to the 20-25s running entry window and does not change normal signals.
    """
    if frame is None or len(frame) < 27:
        return Signal("NO_TRADE", 0, 0, "Running candle-এর জন্য পর্যাপ্ত data নেই।")

    last = frame.iloc[-1]
    close = float(last["close"])
    candle_dir = _dir(last)
    atr = _atr(frame.iloc[:-1])
    candidates = []
    for side, want_dir in (("BUY", "bullish"), ("SELL", "bearish")):
        mirror = get_mirror_projection(frame, side, 20)
        if not mirror or candle_dir != want_dir:
            continue
        tol = max(float(mirror["tolerance"]) * 4.0, atr * 0.60)
        distance = abs(close - float(mirror["zone"]))
        if distance > tol:
            continue
        sweep = liquidity_sweep(frame, 10)
        imp = displacement(frame)
        live_confirm = sweep == ("buy_side_rejection" if side == "BUY" else "sell_side_rejection") or imp == want_dir
        if not live_confirm:
            # A strong directional running candle at the Mirror level is enough
            # for this short entry window when it is already inside the level zone.
            live_confirm = distance <= max(float(mirror["tolerance"]) * 1.5, atr * 0.25)
        if live_confirm:
            candidates.append((distance, side, mirror, sweep, imp))

    if not candidates:
        return Signal("NO_TRADE", 0, 0, "Mirror MMC level/price proximity/current-candle direction/confirmation একসঙ্গে মেলেনি।")
    candidates.sort(key=lambda x: x[0])
    distance, side, mirror, sweep, imp = candidates[0]
    return Signal(side, 0, 0, f"Running MMC fallback: {side} current 1M candle direction + Mirror level proximity confirmed; sweep={sweep}, displacement={imp}, distance={distance:.8g}.")
...
def get_running_signal(pair: str, market_mode: str = "real") -> dict:
    ...
    if sec > ENTRY_END:
        return {"ok": True, "eligible": False, "signal": "NO_ENTRY", "pair": pair, "market_mode": mode,
                ...
                "signal_time_utc": now.isoformat(timespec="seconds"),
                "message": "এই 1-minute candle-এর নতুন entry window শেষ। পরের candle-এর 20-25s window অপেক্ষা করুন।"}
    ...
    if result.action not in {"BUY", "SELL"}:
        fallback = _running_mmc_fallback(frame)
        if fallback.action in {"BUY", "SELL"}:
            result = fallback
            fallback_used = True
    ...
    if loss_reason and result.action in {"BUY", "SELL"}:
        log.info("RUNNING_ENTRY_BLOCK pair=%s mode=%s sec=%s action=%s reason=loss_lock:%s", pair, mode, sec, result.action, loss_reason)
    ...
    if result.action in {"BUY", "SELL"}:
        ...
        if block_reason:
            log.info("RUNNING_ENTRY_BLOCK pair=%s mode=%s sec=%s action=%s reason=reentry:%s", pair, mode, sec, result.action, block_reason)
            result = Signal("NO_TRADE", result.buy_score, result.sell_score, block_reason)
            level_info = None

    log.info("RUNNING_ENTRY_DECISION pair=%s mode=%s sec=%s primary=%s final=%s buy=%s sell=%s fallback=%s reason=%s", pair, mode, sec, primary_action, result.action, result.buy_score, result.sell_score, fallback_used, result.reason)
    ...
    if action in {"BUY", "SELL"}:
        reason = f"{reason} Running 1M candle analysis থেকে {sec}s-এ entry। Entry window: {ENTRY_START}-{ENTRY_END}s; 25s-এর পরে নতুন entry বন্ধ।"
    return {"ok": True, "eligible": True, "pair": pair, "market_mode": mode,
            ...
            "running_fallback": fallback_used}
