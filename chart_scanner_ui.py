...
def _analyze_image(data_url: str, analysis_time_bd: str, pair: str) -> dict:
    ...
    if not api_key:
        return {
            "ok": False,
            "setup_required": True,
            "error": "Chart Scanner-এর জন্য OPENAI_API_KEY এখনো Render-এ সেট করা হয়নি।",
        }
...
.chart-scanner-head{display:flex;justify-content:space-between;align-items:center}.chart-scanner-head h3{margin:0;font-size:22px}.chart-scanner-close{background:#172033!important;padding:7px 10px!important}
.chart-scanner-input{margin:14px 0;padding:14px;border:2px dashed #b8cffd;border-radius:12px;background:#f8fbff;text-align:center}.chart-scanner-input input{max-width:100%;font-size:14px}
#chart-scanner-preview{display:none;max-width:100%;max-height:300px;margin:12px auto;border-radius:10px;border:1px solid #dbe4f0}.chart-scan-btn{width:100%;margin-top:8px}.chart-scan-result{margin-top:14px;padding:14px;border-radius:12px;background:#f8fafc;border:1px solid #dbe4f0}.chart-scan-signal{font-size:30px;font-weight:900;text-align:center;color:#174ea6}.chart-scan-meta{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px}.chart-scan-meta div{padding:9px;background:#fff;border:1px solid #e2e8f0;border-radius:8px}.chart-scan-note{margin-top:10px;font-size:12px;color:#64748b;line-height:1.45}
</style>
"""
        ui = """
<button type="button" id="chart-scanner-trigger">📷 CHART SCANNER</button>
<div id="chart-scanner-overlay" role="dialog" aria-modal="true">
 <div class="chart-scanner-card">
  ...
  <div class="chart-scanner-input">
   <div><strong>Chart screenshot / Camera</strong></div>
   <input id="chart-scanner-file" type="file" accept="image/*" capture="environment">
   <img id="chart-scanner-preview" alt="Chart preview">
   <button type="button" class="chart-scan-btn" id="chart-scanner-analyze">ANALYZE CHART</button>
  </div>
  <div id="chart-scanner-result"></div>
  <div class="chart-scanner-note">এটি independent visual scanner। এটি বর্তমান v2.1 tick strategy পরিবর্তন বা override করে না। Image-ভিত্তিক direction ভুলও হতে পারে—বিশেষ করে ছোট লেখা/দাম বা অসম্পূর্ণ chart crop হলে।</div>
 </div>
</div>
<script>
(()=>{const t=document.getElementById('chart-scanner-trigger');if(!t)return;const o=document.getElementById('chart-scanner-overlay'),c=document.getElementById('chart-scanner-close'),f=document.getElementById('chart-scanner-file'),p=document.getElementById('chart-scanner-preview'),b=document.getElementById('chart-scanner-analyze'),r=document.getElementById('chart-scanner-result');const esc=v=>String(v??'—').replace(/[&<>\\"']/g,x=>({'&':'&amp;','<':'&lt;','>':'&gt;','\\"':'&quot;',"'":'&#39;'}[x]));t.onclick=()=>o.classList.add('open');c.onclick=()=>o.classList.remove('open');o.onclick=e=>{if(e.target===o)o.classList.remove('open')};f.onchange=()=>{const file=f.files?.[0];if(!file)return;p.src=URL.createObjectURL(file);p.style.display='block';r.innerHTML=''};b.onclick=async()=>{const file=f.files?.[0];if(!file){r.innerHTML='<div class="chart-scan-result">প্রথমে chart screenshot/camera image দিন।</div>';return}b.disabled=true;b.textContent='ANALYZING…';r.innerHTML='<div class="chart-scan-result">Chart বিশ্লেষণ করা হচ্ছে…</div>';try{const fd=new FormData();fd.append('chart',file);fd.append('mode',document.getElementById('mode')?.value||'real');fd.append('pair',document.getElementById('pair')?.value||'');const q=await fetch('/chart-scanner',{method:'POST',body:fd,credentials:'same-origin'}),d=await q.json();if(!q.ok||!d.ok)throw Error(d.error||'Scanner failed');const color=d.signal==='BUY'?'#16803c':d.signal==='SELL'?'#dc2626':'#b45309';r.innerHTML=`<div class="chart-scan-result"><div class="chart-scan-signal" style="color:${color}">${esc(d.signal)}</div><div class="chart-scan-meta"><div><b>Signal Time</b><br>${esc(d.signal_time_bd)}</div><div><b>Confidence</b><br>${esc(d.confidence)}%</div><div><b>Trend</b><br>${esc(d.trend)}</div><div><b>Risk</b><br>${esc(d.risk)}</div></div><p><b>Reason:</b> ${esc(d.reason)}</p></div>`}catch(e){r.innerHTML=`<div class="chart-scan-result">${esc(e.message)}</div>`}finally{b.disabled=false;b.textContent='ANALYZE CHART'}}})();
</script>
"""
